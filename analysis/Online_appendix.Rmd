...
plot_ly(z = df, type = "surface") %>% 
  ...
  add_trace(text = "Prediction", name = "Prediction", data = df,  x = x, y = y, z = z_pred, type="mesh3d") %>%
...
## Lagged effects multilevel models
...
tab2 <- tab_model(c(mphys_random_thriv_lagged, mphys_randomX_Y_lagged), show.ci = FALSE, show.se = TRUE, digits.re = 3, show.std = TRUE, 
                  ...
                  order.terms = c(1, 21:25, 2:9, 26:30, 10:20), 
...
```{r}
mment_random_thriv_lagged<- lmer(Mental.health ~1 +lag1.Thriving+
                        lag1.Positive.affect+ lag1.Negative.affect + lag1.Demands+ lag1.Autonomy + 
                        lag1.Coworker.support + lag1.Supervisor.support + lag1.Time + 
                          Mean.thriving +
                        Mean.positive.affect + Mean.negative.affect+ Mean.demands+ 
                          Mean.autonomy+ Mean.coworker.support+ Mean.supervisor.support +  
                          Age + Gender + Education + Organizational.tenure +
