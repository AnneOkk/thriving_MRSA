...
html_table <- corrs %>% add_rownames(.) %>% flextable(.)  %>% height(., height = .4) 
...
longer_comp_l <- longer_comp_l %>% 
  dplyr::rename(., 
                Vitality = x.c,
                Learning = y.c,
                "Vitality.squared" = x2.c,
                "Vitality.x.learning" = xy.c,
                "Learning.squared" = y2.c,
                "Positive.affect" = CWC_pa,
                "Negative.affect" = CWC_na,
                "Demands" = CWC_demands,
                "Mean.vitality" = x.mean,
                "Mental.health" = sf12mcs,
                "Physical.health" = sf12pcs,
                Thriving = thriv.c,
                "Mean.learning" = y.mean,
                "Mean.vitality.squared" = x2.mean,
                "Mean.learning.squared" = y2.mean,
                "Autonomy" = CWC_aut,
                ...
                "Mean.thriving" = thriv_mean,
                "Mean.demands" = demands.mean,
                "Mean.autonomy" = aut.mean,
                "Mean.coworker.support" = csup.mean,
                "Mean.supervisor.support" = ssup.mean,
                "Mean.vitality.x.learning" = xy.mean)

longer_comp_l <- longer_comp_l %>% select(-learn, -vitality, -thriv.mean, -CWC_sf12pcs, -CWC_sf12mcs, -CWC_thriv, -CWC_vitality, -CWC_learn, -ID.mean, -sf12pcs.mean, -vitality.mean, -sf12mcs.mean, -learn.mean)
...
mphys_randomX_Y <- lmer(sf12pcs ~1 +x.c+y.c+x2.c+xy.c+y2.c+
                        ...
                        pa.mean+ na.mean + demands.mean + aut.mean + csup.mean + ssup.mean + set +
                        (1+x.c|ID),data=longer_comp,REML=F, control=lmerControl(optimizer="bobyqa"))
...
MLRSA_AverageSurface(mphys_randomX_Y,name_vars=c("x.c","y.c","x2.c","xy.c","y2.c"),random_vars=c("x.c",NA,NA,NA,NA)) 
check_collinearity(mphys_randomX_Y, sort = T)
...
mphys_randomX_Y_lagged<- lmer(Physical.health ~1 +Vitality+Learning+ Vitality.squared+ Vitality.x.learning +Learning.squared+
                        ...
                        Mean.positive.affect + Mean.negative.affect+ Mean.demands+ 
                          ...
                          Age + Gender + Education + Organizational.tenure +
...
std_coef_ment_thriv <- stdCoef.merMod(mment_randomthriv)
...
MLRSA_AverageSurface(mment_randomX_Y,name_vars=c("x.c","y.c","x2.c","xy.c","y2.c"),random_vars=c("x.c",NA,NA,NA,NA)) 
check_collinearity(mment_randomX_Y, sort = T)
