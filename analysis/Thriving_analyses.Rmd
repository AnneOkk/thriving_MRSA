...
corstar <- data.frame(corstars_no_stars(cor_tab, removeTriangle = "none", result="none"))
...
model <- '
    level: 1
        ...
        csup_w =~ csup1 + csup2 + csup3 + csup4
        ssup_w =~ ssup1 + ssup2 + ssup3 + ssup4
        pa_w =~ pa1 + pa2 + pa3 + pa4 + pa5
        na_w =~ na1 + na2 + na3 + na4 + na5
        phys_w  =~ sf12pf + sf12rp + sf12bp + sf12gh
        ment_w  =~ sf12vt + sf12sf + sf12re + sf12mh
    ...
    level: 2
        ...
        csup_b =~ csup1 + csup2 + csup3 + csup4
        ssup_b =~ ssup1 + ssup2 + ssup3 + ssup4
        pa_b =~ pa1 + pa2 + pa3 + pa4 + pa5
        na_b =~ na1 + na2 + na3 + na4 + na5
        phys_b  =~ sf12pf + sf12rp + sf12bp + sf12gh
        ment_b  =~ sf12vt + sf12sf + sf12re + sf12mh
        
'

model_thriv <- '
    level: 1
        thriv_w =~ thriv1 + thriv2 + thriv3 + thriv4 + thriv5 + thriv6

    level: 2
        thriv_b =~ thriv1 + thriv2 + thriv3 + thriv4 + thriv5 + thriv6
'
...
fit_thriv <- sem(model_thriv, data = longer, cluster = "ID", std.lv  = TRUE, verbose = FALSE, estimator="MLR")

rels_within_thriv <- reliability(fit_thriv, what = c("alpha", "omega", "omega2", "omega3", "ave"),
  return.total = FALSE, dropSingle = TRUE, omit.factors = character(0),
  omit.indicators = character(0), omit.imps = c("no.conv", "no.se"))$within[1, ] %>% round(., 2) %>%  gsub("0.", ".", .)

rels_within <- reliability(fit, what = c("alpha", "omega", "omega2", "omega3", "ave"),
  return.total = FALSE, dropSingle = TRUE, omit.factors = character(0),
  omit.indicators = character(0), omit.imps = c("no.conv", "no.se"))$within[1, ] %>% round(., 2) %>% c(rels_within_thriv, .) %>% .[c(2, 3, 1, 4: 9)] %>%  gsub("0.", ".", .)

rels_between_thriv <- reliability(fit_thriv, what = c("alpha", "omega", "omega2", "omega3", "ave"),
  return.total = FALSE, dropSingle = TRUE, omit.factors = character(0),
  omit.indicators = character(0), omit.imps = c("no.conv", "no.se"))$ID[1, ] %>% round(., 2) %>% gsub("0.", ".", .)

rels_between <- reliability(fit, what = c("alpha", "omega", "omega2", "omega3", "ave"),
  return.total = FALSE, dropSingle = TRUE, omit.factors = character(0),
  omit.indicators = character(0), omit.imps = c("no.conv", "no.se"))$ID[1, ] %>% round(., 2) %>% c(rels_between_thriv, .)  %>% .[c(2, 3, 1, 4: 9)] %>% 
  gsub("0.", ".", .)

rels <- cbind(rels_within, rels_between) %>% rbind(., matrix(data="", ncol=2, nrow=6))
...
```{r eval = F}
library(psychTools)
longer$set <- as.numeric(gsub("T|_", "", longer$set))
longer_T1_T2 <- longer %>% filter(set == 2 | set == 3)
pcs <- longer_T1_T2 %>% select(matches("sf12pf|sf12rp|sf12bp|sf12gh")) %>% names(.)
testre <- testRetest(longer_T1_T2, select = pcs, time = "set", id = "ID")
summary(testre)
...
# Multilevel Reliabilities - single health items 
...
rels_within <- reliability(fit, what = c("alpha", "omega", "omega2", "omega3", "ave"),
  return.total = FALSE, dropSingle = TRUE, omit.factors = character(0),
  omit.indicators = character(0), omit.imps = c("no.conv", "no.se"))$within[1, ] %>% round(., 2)

rels_between <- reliability(fit, what = c("alpha", "omega", "omega2", "omega3", "ave"),
  return.total = FALSE, dropSingle = TRUE, omit.factors = character(0),
  omit.indicators = character(0), omit.imps = c("no.conv", "no.se"))$ID[1, ] %>% round(., 2) 
...
# Multilevel Reliabilities - original health items 

```{r}

SF <- dataset3 %>%  mutate(T2_SF12 = (T2_SF12*(-1) + 6)) %>%  mutate(T2_SF13 = (T2_SF13*(-1) + 6)) %>% 
  mutate(T3_SF12 = (T3_SF12*(-1) + 6)) %>%  mutate(T3_SF13 = (T3_SF13*(-1) + 6)) %>% 
  mutate(T4_SF12 = (T4_SF12*(-1) + 6)) %>%  mutate(T4_SF13 = (T4_SF13*(-1) + 6)) %>% 
  mutate(T5_SF12 = (T5_SF12*(-1) + 6)) %>%  mutate(T5_SF13 = (T5_SF13*(-1) + 6)) %>% 
  select(matches("T\\d_SF\\d{1,2}$", ignore.case = F)) %>% scale(.) %>% as.data.frame(.)

phys_heal <- SF %>%  select(T2_SF1, T2_SF11, T2_SF2, T2_SF3, T2_SF4, T2_SF5)
psych::alpha(phys_heal)

ment_heal <- SF %>%  select(T2_SF12, T2_SF13, T2_SF14, T2_SF15, T2_SF6, T2_SF7)
psych::alpha(ment_heal)

```


```{r}
SF$ID <- seq.int(nrow(SF))
...
model <- '
    level: 1
        phys_w  =~ SF1 + SF11 + SF2 + SF3 + SF4 + SF5
        ment_w  =~ SF12 + SF13 + SF14 + SF15 + SF6 + SF7


        
    level: 2
        phys_b  =~ SF1 + SF11 + SF2 + SF3 + SF4 + SF5
        ment_b  =~ SF12 + SF13 + SF14 + SF15 + SF6 + SF7
'
...
fit1 <- sem(model, data = longer, cluster = "ID", std.lv  = TRUE, verbose = FALSE, estimator="MLR")
summary(fit1, fit.measures=TRUE)
```

## Vita and learn combined
...
fit2 <- sem(model, data = longer, cluster = "ID", std.lv  = TRUE, verbose = FALSE, estimator="MLR")
...
fit3 <- sem(model, data = longer, cluster = "ID", std.lv  = TRUE, verbose = FALSE, estimator="MLR")
...
model <- '
    level: 1
        vita_w  =~  vitality1 + vitality2 + vitality5 + sf12pcs
        learn_w =~ learn1 + learn2 + learn3
        ment_w  =~ sf12mcs
        sf12mcs ~~ 0*sf12mcs
    ...
    level: 2
        vita_b  =~  vitality1 + vitality2 + vitality5 + sf12pcs
        learn_b =~ learn1 + learn2 + learn3
        ment_b  =~ sf12mcs
        sf12mcs ~~ 0*sf12mcs
...
fit4 <- sem(model, data = longer, cluster = "ID", std.lv  = TRUE, verbose = FALSE, estimator="MLR")
...
model <- '
    level: 1
        vita_w  =~  vitality1 + vitality2 + vitality5 
        learn_w =~ learn1 + learn2 + learn3 + sf12pcs
        ment_w  =~ sf12mcs
        sf12mcs ~~ 0*sf12mcs
    ...
    level: 2
        vita_b  =~  vitality1 + vitality2 + vitality5 
        learn_b =~ learn1 + learn2 + learn3 + sf12pcs
        ment_b  =~ sf12mcs
        sf12mcs ~~ 0*sf12mcs
        
'

fit5 <- sem(model, data = longer, cluster = "ID", std.lv  = TRUE, verbose = FALSE, estimator="MLR")
...
model <- '
    level: 1
        vita_w  =~  vitality1 + vitality2 + vitality5 
        learn_w =~ learn1 + learn2 + learn3 + sf12mcs
        phys_w  =~ sf12pcs
        sf12pcs ~~ 0*sf12pcs
    ...
    level: 2
        vita_b  =~  vitality1 + vitality2 + vitality5 
        learn_b =~ learn1 + learn2 + learn3 + sf12mcs
        phys_b  =~ sf12pcs
        sf12pcs ~~ 0*sf12pcs
        
'

fit6 <- sem(model, data = longer, cluster = "ID", std.lv  = TRUE, verbose = FALSE, estimator="MLR")
...
model <- '
    ...
    level: 2
        vita_b  =~  vitality1 + vitality2 + vitality5 
        learn_b =~ learn1 + learn2 + learn3 + sf12mcs
        phys_b  =~ sf12pcs + sf12mcs
        sf12pcs ~~ 0*sf12pcs
        sf12mcs ~~ 0*sf12mcs
        
'

fit7 <- sem(model, data = longer, cluster = "ID", std.lv  = TRUE, verbose = FALSE, estimator="MLR")
...
fits <- compareFit(fit1, fit2)
summary(fits)

fits2 <- compareFit(fit1, fit3)
summary(fits2)

fits3 <- compareFit(fit1, fit4)
summary(fits3)

fits2 <- compareFit(fit, fit5)
summary(fits2)

fits2 <- compareFit(fit, fit6)
summary(fits2)

fits2 <- compareFit(fit, fit7)
summary(fits2)

fits2 <- compareFit(fit, fit3)
summary(fits2)
```
...
# save raw data for later use to make lagged df and corr table
longer_comp_raw <- longer_comp

## Create polynomial terms
dat <- make_polys(longer_comp) %>% select(matches("^x|^y|.mean$", ignore.case = F)) %>% drop_na(x, y) 

# create thriving centered and thriving mean for use in multilevel RSA
dat$thriv <- (dat$x + dat$y)/2
dat$thriv.mean <- (dat$x.mean + dat$y.mean)/2

# select variables from full df
longer_comp <- longer_comp %>% drop_na(vitality, learn) %>% dplyr::select(where(is.numeric))

# create aggregate_df for calculating means 
aggregate_df <- longer_comp %>%
  group_by(ID) %>%
  summarise_at(vars(-set), funs(mean(., na.rm=TRUE))) %>% dplyr::select(ID, na, pa, demands, aut, csup, ssup, sf12pcs, sf12mcs, vitality, learn)
names(aggregate_df) <- paste0(names(aggregate_df), ".mean")
aggregate_df <- aggregate_df %>% dplyr::rename("ID" = "ID.mean")
aggregate_df$vitality2.mean <- aggregate_df$vitality.mean^2
aggregate_df$learn2.mean <- aggregate_df$learn.mean^2
aggregate_df$vitalitylearn.mean <- aggregate_df$vitality.mean*aggregate_df$learn.mean
aggregate_df$thriv.mean <- (aggregate_df$learn.mean + aggregate_df$vitality.mean)/2
...
# df with the two outcome variables and time
health <- longer_comp %>% select(sf12pcs, sf12mcs, set)

# group mean center change variables for use in multilevel model
cwc_df <- longer_comp %>% select(ID, aut, csup, ssup, demands, na, pa) %>% cbind(., dat) %>% select(-matches(".mean"))
...
longer_comp <- cbind(cwc_df, gmc_df, health)
...
write.csv(longer_comp, "../../data/longer_comp.csv")

```

# Correlation table multilevel

```{r}

longer_comp_corr <- longer_comp_raw %>% 
  select(ID, 
         ends_with("vitality"), ends_with("learn"), ends_with("thriv"),
         aut, demands, csup, ssup, pa, na,
         ends_with("12pcs"),  ends_with("12mcs"),
         T1_age:T1_edu, T1_tenure_org) 

within_between <- statsBy(longer_comp_corr, "ID", cors = FALSE, method="pearson",  use="pairwise")

print(within_between, short = FALSE)
within <- within_between$rwg
between <- within_between$rbg

str(within_between)


within_between$ci.wg$r.ci %>% arrange(., abs(r)) # all > 0.04 are sign at .05
within_between$ci.bg$r.ci %>% arrange(., abs(r)) # all > 0.06 are sign at .05

corstar_within <- as.matrix(corstars_no_stars(within, removeTriangle = "lower", result="none"))
corstar_between <- as.matrix(corstars_no_stars(between, removeTriangle = "upper", result="none"))

corstar_between[upper.tri(corstar_between)] <- corstar_within[upper.tri(corstar_within)]
...
# Crate cortable with additional info 
```{r}

mean <- longer_comp_corr %>% colMeans(., na.rm = T) %>% round(.,2) %>% .[-1]
sd <- cbind(apply(longer_comp_corr, 2, sd, na.rm = T)) %>% round(., 2) %>% .[-1]
mean_sd <- cbind(mean,sd)

corrs <- as.data.frame(corstar_between) %>% cbind(ICC_12, rels, .)
corrs <- rbind(corrs, c("", "", "", "", mean), c("", "", "", "", sd))

html_table <- corrs %>% tibble::rownames_to_column(.) %>% flextable(.)  %>% height(., height = .4) 
 
html_table <- width(html_table, width = 0.4)
doc <- read_docx() %>%
   body_add_flextable(value = html_table, split = TRUE) %>%
   body_end_section_landscape() %>% # a landscape section is ending here
   print( target = "../output/my_table_corr.docx" )
```
...
dataframe_l$T2_newsf12mcs <- dataframe_l$T2_sf12mcs
dataframe_l$T3_newsf12mcs <- dataframe_l$T3_sf12mcs
dataframe_l$T4_newsf12mcs <- dataframe_l$T4_sf12mcs
dataframe_l$T5_newsf12mcs <- dataframe_l$T5_sf12mcs
dataframe_l$T2_newsf12pcs <- dataframe_l$T2_sf12pcs
dataframe_l$T3_newsf12pcs <- dataframe_l$T3_sf12pcs
dataframe_l$T4_newsf12pcs <- dataframe_l$T4_sf12pcs
dataframe_l$T5_newsf12pcs <- dataframe_l$T5_sf12pcs
...
## Create polynomial terms
dat <- make_polys(longer_comp_l) %>% select(matches("^x|^y|.mean$", ignore.case = F)) %>% drop_na(x, y) 
...
# select variables from full df
longer_comp_l <- longer_comp_l %>% drop_na(vitality, learn) %>% dplyr::select(where(is.numeric))
...
# df with the two outcome variables and time
health <- longer_comp_l %>% select(sf12pcs, sf12mcs, set)

# group mean center change variables for use in multilevel model
cwc_df <- longer_comp_l %>% select(ID, aut, csup, ssup, demands, na, pa, newsf12mcs, newsf12pcs) %>% cbind(., dat) %>% select(-matches(".mean"))
cwc_df <- as.data.frame(lapply(cwc_df[-1], center, type = "CWC", cluster = cwc_df$ID))
names(cwc_df) <- paste0("CWC_", names(cwc_df))
cwc_df$set <- cwc_df$CWC_set
cwc_df$ID <- longer_comp_l$ID

longer_comp_l <- cbind(cwc_df, gmc_df, health)
...
longer_comp_l <- longer_comp_l %>% 
  dplyr::rename(., 
                Vitality = CWC_x,
                Learning = CWC_y,
                "Vitality.squared" = CWC_x2,
                "Vitality.x.learning" = CWC_xy,
                "Learning.squared" = CWC_y2,
                "Positive.affect" = CWC_pa,
                "Negative.affect" = CWC_na,
                "Demands" = CWC_demands,
                "Mean.vitality" = vitality.mean,
                "Mental.health" = sf12mcs,
                "Physical.health" = sf12pcs,
                Thriving = CWC_thriv,
                "Mean.learning" = learn.mean,
                "Mean.vitality.squared" = vitality2.mean,
                "Mean.learning.squared" = learn2.mean,
                "Autonomy" = CWC_aut,
                "Coworker.support" = CWC_csup,
                "Supervisor.support" = CWC_ssup,
                "Time" = set,
                "Person.ID" = ID,
                "Age" = GMC_T1_age,
                "Gender" = GMC_T1_sex,
                "Education" = GMC_T1_edu,
                "Organizational.tenure" = GMC_T1_tenure_org,
                "Mean.negative.affect" = na.mean,
                "Mean.positive.affect" = pa.mean,
                "Mean.thriving" = thriv.mean,
                "Mean.demands" = demands.mean,
                "Mean.autonomy" = aut.mean,
                "Mean.coworker.support" = csup.mean,
                "Mean.supervisor.support" = ssup.mean,
                "Mean.vitality.x.learning" = vitalitylearn.mean,
                "T-1.Physical.health" = CWC_newsf12pcs,
                "T-1.Mental.health" = CWC_newsf12mcs)

write.csv(longer_comp_l, "../../data/longer_comp_l.csv")
```
...
tab_model(mphys_randomthriv, show.ci = FALSE, show.se = TRUE, digits.re = 3)
...
std_coef_phys_thriv <- stdCoef.merMod(mphys_randomthriv)
...
tab_model(mphys_randomX_Y, show.ci = FALSE, show.se = TRUE, digits.re = 3)
...
MLRSA_AverageSurface(mphys_randomX_Y,name_vars=c("CWC_x","CWC_y","CWC_x2","CWC_xy","CWC_y2"),random_vars=c("CWC_x",NA,NA,NA,NA)) 
check_collinearity(mphys_randomX_Y, sort = T)

# PLOT THE ESTIMATED MODEL
# pdf(file = "Physheal.pdf")
quantile(longer_comp$CWC_x, probs = c(0.05, 0.95))
quantile(longer_comp$CWC_y, probs = c(0.05, 0.95))

quantile(longer_comp$sf12pcs, probs = c(0.05, 0.95), na.rm = T)

MLRSA_AverageSurfacePlot(mphys_randomX_Y,name_vars=c("CWC_x","CWC_y","CWC_x2","CWC_xy","CWC_y2"),
...
plot <- plotRSA(b0=b0, x=b1, y=b2, x2=b3, xy=b4, y2=b5,
                type = "3d", surface = NULL, zlim=c(3.2, 5.9),
                points = 
...
summary(mphys_randomX_Y_lagged)

tab_model(mphys_randomX_Y_lagged, show.ci = FALSE, show.se = TRUE, digits.re = 3)

```


### Mental health by thriving
...
tab_model(mment_randomthriv, show.ci = FALSE, show.se = TRUE, digits.re = 3)
...
std_coef_phys_thriv <- stdCoef.merMod(mment_randomthriv)
...
tab_model(mment_randomX_Y, show.ci = FALSE, show.se = TRUE, digits.re = 3)

std_coef_ment_XY <- stdCoef.merMod(mment_randomX_Y)

cat("R square marginal =" ,r.squaredGLMM(mment_randomX_Y)[1,1])
cat("R square conditional =" ,r.squaredGLMM(mment_randomX_Y)[1,2])

MLRSA_AverageSurface(mment_randomX_Y,name_vars=c("CWC_x","CWC_y","CWC_x2","CWC_xy","CWC_y2"),random_vars=c("CWC_x",NA,NA,NA,NA)) 
check_collinearity(mment_randomX_Y, sort = T)

# PLOT THE ESTIMATED MODEL
# pdf(file = "mentheal.pdf")
quantile(longer_comp$CWC_x, probs = c(0.05, 0.95))
quantile(longer_comp$CWC_y, probs = c(0.05, 0.95))

quantile(longer_comp$sf12mcs, probs = c(0.05, 0.95), na.rm = T)

MLRSA_AverageSurfacePlot(mment_randomX_Y,name_vars=c("CWC_x","CWC_y","CWC_x2","CWC_xy","CWC_y2"),
                         outcome="sf12mcs",data=longer_comp, type = "interactive",
                         axesStyles=list(LOC = list(lty="solid", lwd=2, col= "black")),
...
plot <- plotRSA(b0=b0, x=b1, y=b2, x2=b3, xy=b4, y2=b5,
                type = "3d", surface = NULL, zlim = c(2.8, 5.9),
                points = 
                  list(data = plotting_df, show = NA, value = "raw", jitter = 0.005, 
                       color = "transparent", cex = 0.07, out.mark = FALSE),
                xlab = "Vitality", ylab = "Learning", zlab = "Mental Health",
                pad = c(1,2,-3), bw = T, xlim = c(-3,3), ylim = c(-3,3), suppress.grid = F, 
                maxlines = F
                )
plot
dev.off
...
summary(mment_randomX_Y_lagged)

tab_model(mment_randomX_Y_lagged, show.ci = FALSE, show.se = TRUE, digits.re = 3)
