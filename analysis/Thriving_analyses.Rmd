...
corstar <- data.frame(corstars(cor_tab, removeTriangle = "upper", result="none"))
...
cor_tab <- comp_df %>% dplyr::select(matches("_age|_sex|edu$|tenure_org|thriv|vita|learn|sf12pcs|sf12mcs")) 
...
doc <- read_docx() %>%
   body_add_flextable(value = html_table, split = TRUE) %>%
   body_end_section_landscape() %>% # a landscape section is ending here
   print( target = "my_table_short.docx" )
...
# Calculating ICC
library("merTools")
(ICC(outcome = "vitality1", group = "ID", data = longer) +
ICC(outcome = "vitality2", group = "ID", data = longer) +
ICC(outcome = "vitality5", group = "ID", data = longer) +
ICC(outcome = "learn1", group = "ID", data = longer) +
ICC(outcome = "learn2", group = "ID", data = longer) +
ICC(outcome = "learn3", group = "ID", data = longer)) / 6
...
longer_comp <- longer_comp %>% drop_na(vitality, learn, sf12mcs, sf12pcs) 
...
cwc_df <- cwc_df %>% dplyr::select(matches("CWC|ID|set"))
...
fit1 <- sem(model, data = longer, cluster = "ID",
...
mphys_randomX_Y <- lmer(sf12pcs ~1 +CWC_vitality+ CWC_learn +
                        ...
                        pa.mean+ na.mean + demands.mean + aut.mean + csup.mean + ssup.mean +
...
MLRSA_AverageSurfacePlot(mphys_randomX_Y,name_vars=c("X","Y","X2","XY","Y2"),
                          outcome="sf12pcs",data=longer_comp,
...
```{r}
mphys_randomX_Y <- lmer(sf12mcs ~1 +CWC_vitality+ CWC_learn +
                        GMC_T1_age+ GMC_T1_sex + GMC_T1_edu+ GMC_T1_tenure_org + 
                        CWC_pa + CWC_na + CWC_demands + CWC_aut + CWC_csup + CWC_ssup +
                        vitality.mean + learn.mean + 
                        pa.mean+ na.mean + demands.mean + aut.mean + csup.mean + ssup.mean +
...
# Physical health by thriving
mphys_randomthriv <- lmer(sf12mcs~1 +CWC_thriv+
                        GMC_T1_age+ GMC_T1_sex + GMC_T1_edu+ GMC_T1_tenure_org + set +
                        CWC_pa + CWC_na + CWC_demands + CWC_aut + CWC_csup + CWC_ssup +
                        thriv_mean + 
                        pa.mean+ na.mean + demands.mean + aut.mean + csup.mean + ssup.mean + set +
...
```{r}
mphys_randomX_Y <- lmer(sf12mcs ~1 +x.c+y.c+x2.c+xy.c+y2.c+
                        GMC_T1_age+ GMC_T1_sex + GMC_T1_edu+ GMC_T1_tenure_org + 
                        CWC_pa + CWC_na + CWC_demands + CWC_aut + CWC_csup + CWC_ssup +
                        x.mean+y.mean+x2.mean+xy.mean+y2.mean+ 
                        pa.mean+ na.mean + demands.mean + aut.mean + csup.mean + ssup.mean + set +
...
```{r}
longer_comp_nona <- longer_comp %>% drop_na(X, Y, sf12mcs, sf12pcs) 
longer_comp_nona <- longer_comp_nona[!duplicated(longer_comp_nona$ID),]

library(janitor)
# gender
length(longer_comp_nona$T1_sex)
tabyl(longer_comp_nona$T1_sex)

#age
#Ranges 
# age range 18-90
longer_comp_nona <- longer_comp_nona %>% replace_with_na(replace = list(T1_age = c(7)))

summary(longer_comp_nona$T1_age)
sd(longer_comp_nona$T1_age, na.rm =T)


# education 
longer_comp_nona
tabyl(longer_comp_nona$T1_edu)

### industries 
tabyl(longer_comp_nona$T1_industry, sort = T) %>% arrange(desc(n))

## job tenure 
summary(longer_comp_nona$T1_tenure_org)
sd(longer_comp_nona$T1_tenure_org, na.rm=T)

names(longer_comp_nona)
...
X = dataframe$T2_vitality_comp
Y = dataframe$T2_learn_comp

# center vit and learn  at the grand mean 
grandmean <- mean(c(X, Y), na.rm=T)
X <- X-grandmean
Y <- Y-grandmean
...
names(dataframe)
data_vif = data.frame(dataframe$T2_phys_comp,  dataframe$T2_ment_comp, 
                      dataframe$T3_phys_comp,  dataframe$T3_ment_comp, 
                      X, Y, X2, XY, Y2
                      )
names(data_vif) <- sub(".*\\.", "", names(data_vif))

library(car)
names(data_vif)

lm_phys <- lm(T3_phys_comp ~ X + Y + X2 + XY + Y2 + T2_phys_comp, data=data_vif)
vif(lm_phys)

lm_ment <- lm(T3_ment_comp ~ X + Y + X2 + XY + Y2 + T2_ment_comp, data=data_vif)
vif(lm_ment)

VIF <- cbind(vif(lm_phys), vif(lm_ment))
VIF <- as.data.frame(VIF)
which.min(apply(VIF, 1, max))
which.min(apply(VIF, 2, max))
which.max(apply(VIF, 1, max))
which.max(apply(VIF, 2, max))
...
RICLPM1.ext3 <- '
  ...
  RIphys ~~ RIment + RIvitality + RIlearn + RIpa + RIna + RIdemands + RIaut + RIcsup + RIssup
  RIment ~~ RIvitality + RIlearn + RIpa + RIna + RIdemands + RIaut + RIcsup + RIssup
  RIvitality ~~ RIvitality + RIlearn + RIpa + RIna + RIdemands + RIaut + RIcsup + RIssup
  RIlearn ~~ RIlearn + RIpa + RIna + RIdemands + RIaut + RIcsup + RIssup
  RIpa ~~ RIpa + RIna + RIdemands + RIaut + RIcsup + RIssup
  RIna ~~ RIna + RIdemands + RIaut + RIcsup + RIssup
  RIdemands ~~ RIdemands + RIaut + RIcsup + RIssup
  RIaut ~~ RIaut + RIcsup + RIssup
  RIcsup ~~ RIcsup + RIssup
  RIssup ~~ RIssup
  ...
  WvitalityT2 + WvitalityT3 + WvitalityT4 + WlearnT2 + WlearnT3 + WlearnT4 + WpaT2 + WpaT3 + WpaT4 + WnaT2 + WnaT3 + WnaT4 + WdemandsT2 + WdemandsT3 + WdemandsT4 + WautT2 + WautT3 + WautT4 + WcsupT2 + WcsupT3 + WcsupT4 + WssupT2 + WssupT3 + WssupT4 ~ 1
  ...
  # Regressions
  WphysT2 ~ pv*WvitalityT1 + pl*WlearnT1 + WpaT1 + WnaT1 + WdemandsT1 + WautT1 + WcsupT1 + WssupT1 
  WphysT3 ~ pv*WvitalityT2 + pl*WlearnT2 + WpaT2 + WnaT2 + WdemandsT2 + WautT2 + WcsupT2 + WssupT2 
  WphysT4 ~ pv*WvitalityT3 + pl*WlearnT3 + WpaT3 + WnaT3 + WdemandsT3 + WautT3 + WcsupT3 + WssupT3 
  
  WmentT2 ~ mv*WvitalityT1 + ml*WlearnT1 + WpaT1 + WnaT1 + WdemandsT1 + WautT1 + WcsupT1 + WssupT1 
  WmentT3 ~ mv*WvitalityT2 + ml*WlearnT2 + WpaT2 + WnaT2 + WdemandsT2 + WautT2 + WcsupT2 + WssupT2 
  WmentT4 ~ mv*WvitalityT3 + ml*WlearnT3 + WpaT3 + WnaT3 + WdemandsT3 + WautT3 + WcsupT3 + WssupT3 
  ...
  WphysT1 + WmentT1 + WvitalityT1 ~~ WlearnT1
  WphysT1 + WmentT1 ~~ WvitalityT1
  WphysT1 ~~ WmentT1
  
  WphysT2 + WmentT2 + WvitalityT2 ~~ WlearnT2
  WphysT2 + WmentT2 ~~ WvitalityT2
  WphysT2 ~~ WmentT2
  
  WphysT3 + WmentT3 + WvitalityT3 ~~ WlearnT3
  WphysT3 + WmentT3 ~~ WvitalityT3
  WphysT3 ~~ WmentT3
  
  WphysT4 + WmentT4 + WvitalityT4 ~~ WlearnT4
  WphysT4 + WmentT4 ~~ WvitalityT4
  WphysT4 ~~ WmentT4
  ...
  RIvitality1 + RIvitality2 + RIvitality3 + RIlearn1 + RIlearn2 + RIlearn3 + RIpa1 + RIpa2 + RIpa3 + RIpa4 + RIpa5 + RIna1 + RIna2 + RIna3 + RIna4 + RIna5 + RIdemands1 + RIdemands2 + RIdemands3 + RIaut1 + RIaut2 + RIaut3 + RIcsup1 + RIcsup2 + RIcsup3 + RIcsup4 + RIssup1 + RIssup2 + RIssup3 + RIssup4 + RIphys + RIment ~~ 0*WvitalityT1 + 0*WlearnT1 + 0*WpaT1 + 0*WnaT1 + 0*WdemandsT1 + 0*WautT1 + 0*WcsupT1 + 0*WssupT1 + 0*WphysT1 + 0*WmentT1 
'
